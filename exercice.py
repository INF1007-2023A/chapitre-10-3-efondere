...
MajorChord = collections.namedtuple(
    "MajorChord",
    """
    root,
    third,
...
def merge_channels(channels):
    ...
    return np.fromiter(
        (sample for samples in zip(*channels) for sample in samples), float
    )
...
def generate_sample_time_points(duration):
    # Générer un tableau de points temporels également espacés en seconde. On a SAMPLING_FREQ
    # points par seconde.
    return np.linspace(0, duration, int(SAMPLING_FREQ * duration))
...
def sine(freq, amplitude, duration):
    ...
    # Formule de la valeur y d'une onde sinusoïdale à l'angle x en fonction de sa fréquence F et de
    # son amplitude A :
    # y = A * sin(F * x), où x est en radian.
    # Si on veut le x qui correspond au moment t, on peut dire que 2π représente une seconde, donc
    # x = t * 2π
    # On utilise la fonction précédente pour générer les t
    t = generate_sample_time_points(duration)
    y = amplitude * np.sin(freq * t * 2 * np.pi)
    return y
...
def normalize(samples, norm_target):
    ...
    samples_max = np.max(samples_abs)
    coeff = norm_target / samples_max
    return samples * coeff


def build_major_chord(root_freq, wave_fn, duration):
    # Un accord majeur (racine, tierce, quinte, octave) en intonation juste.
    third_freq, fifth_freq, octave_freq = (
        root_freq * 5 / 4,
        root_freq * 3 / 2,
        root_freq * 2,
    )
...
def apply_fft(sig, sampling_rate):
    ...
    axe_freq = np.linspace(0.0, sampling_rate / 2, sig.size // 2)
    ...
    # la fonction suivante retourne le "nombre" d'echantillons qui ont contribue a chaque freq.
    fourier = np.abs(
        sp.fft.fft(sig)[: sig.size // 2]
    )  # : len(sig) // 2 to remove the imagniary part
    return fourier / (sig.size / 2), axe_freq
...
def main():
    ...
    ys = (
        sine(1.0, 1.0, duration)
        + sine(10.0, 0.2, duration)
        + sine(100.0, 0.2, duration)
    )
    ys = normalize(ys, 0.89)
    ...
    plt.xscale(
        "log"
    )  # echelle log ici! pour que l'espace entre les octaves soit constant
